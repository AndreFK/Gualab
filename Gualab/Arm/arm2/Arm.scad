...
// ISO metric bolt holding hand and arm together (mm)
WristBoltDiameter = 8;//[4, 6, 8, 10]
...
module print_part( ) {
   ...
   if (part == "Cover2") {
    ...
    if(part == "WristBoltNoCover") { make_bolt(WristBoltDiameter, HandScale * 10, 0, WristBoltDiameter  + WristBoltDiameter/2); }
       
    if(part == "WristBoltWithCover") { 
        make_bolt(WristBoltDiameter, HandScale * 10 + ArmScale*3, WristBoltDiameter/3, WristBoltDiameter  + WristBoltDiameter/2);
        
        //Add the ring to hold the cover
        rotate([-90,0,0])   difference() {
            thread_in_ring(WristBoltDiameter  , ArmScale*3 +  WristBoltDiameter/3 , 1); 
            //flatten one side to make it printable
            translate([-(ArmScale*3 +  WristBoltDiameter/3 ), WristBoltDiameter/2 - WristBoltDiameter/12, -1]) cube(ArmScale*3 +  WristBoltDiameter/3  *2);   
            }
        }
...
module MakeArm(PieceNumber) {
        ...
        if(ArmPieces == 1) {
            ...
            if(ArmPieces == 2) {
                ...
                if(PieceNumber == 1)
                    ...
                    //Cut a hole for wrist bolts
        translate([ForeArmCircumferenceScale  * 27.228, ArmScale  * 37.319, ArmScale  * -39.35 ]) rotate([90,0,0])  cylinder(d=WristBoltDiameter + 0.5, h = ArmScale  * 50, center=true, $fn=30);
    
        translate([ForeArmCircumferenceScale  * 27.228 - 85*HandScale, ArmScale  * 37.319, ArmScale  * -39.35 ]) rotate([90,0,0])  cylinder(d=WristBoltDiameter + 0.5, h = ArmScale  * 50, center=true, $fn=30);
    ...
    if((ArmPieces == 4 && PieceNumber == 3) || (ArmPieces == 2 && PieceNumber == 2) || ArmPieces == 1) {
        ...
        difference() {
           ...
           translate([ForeArmCircumferenceScale  * -74.64, ArmScale  * 40.819, ArmScale  * -324.376 ]) rotate([90,0,0]) translate([0, 0, ArmScale * -5 + 10 + 6]) cylinder(d=ElbowBoltDiameter + 0.5, h=ArmScale * 10,center=true, $fn=20);
        }
    }
   ...
   if((ArmPieces == 4 && PieceNumber == 4) || (ArmPieces == 2 && PieceNumber == 2) || ArmPieces == 1) {
       ...
       difference() {
           ...
           translate([ForeArmCircumferenceScale  * 60.36, ArmScale  * 40.819, ArmScale  * -324.376 ]) rotate([90,0,0]) translate([0, 0, ArmScale * -5 + 10 + 6]) cylinder(d=ElbowBoltDiameter + 0.5, h=ArmScale * 10,center=true, $fn=20);
       }
...
module MakeCover1() {
    difference() {
        ...
        if(CoverPieces ==4)
         ...
         //Cut a hole for wrist bolts
        translate([ForeArmCircumferenceScale  * -235.07, ArmScale  * 0, ArmScale  * -44.8 ]) rotate([90,0,0])  cylinder(d=WristBoltDiameter + 0.5, h = ArmScale  * 50, center=true, $fn=30);
    
        translate([ForeArmCircumferenceScale  * -342.255, ArmScale  * 0, ArmScale  * -44.8 ]) rotate([90,0,0])  cylinder(d=WristBoltDiameter + 0.5, h = ArmScale  * 50, center=true, $fn=30);
...
module MakePalm() {
    difference(){
        ...
        // Make threades for wrist bolts
        // I add this mirror because the model will be mirrored at the end if it is a left
        if (LeftRight == "Left") {
                mirror([0,1,0]) translate([HandScale * 1.82,0,HandScale * -12]) rotate([0,90,0]) translate([0, -30 * HandScale,  0]) scale ([1.02,1,1.02])make_bolt(WristBoltDiameter, HandScale * 100, 0, WristBoltDiameter);
        } else {
            translate([HandScale * 1.82,0,HandScale * -12]) rotate([0,90,0]) translate([0, -30 * HandScale,  0]) scale ([1.02,1,1.02])make_bolt(WristBoltDiameter, HandScale * 100, 0, WristBoltDiameter);
        }
...
module MakeThermoform1() {
    ...
    difference(){
        ...
        //Cut hole for first wrist bolt holder
        translate([ForeArmCircumferenceScale * 272.216, ForeArmCircumferenceScale * -19.936, ArmScale * -33]) rotate([10,90,0]) translate([0, 0, -45 * ForeArmCircumferenceScale])  cylinder(d=WristBoltDiameter, h=50 * ForeArmCircumferenceScale,center=false, $fn=20);
       
    }
   
    //Add inner threads for wrist bolt holder
    translate([ForeArmCircumferenceScale * 272.216, ForeArmCircumferenceScale * -19.936, ArmScale * -33]) rotate([10,90,0]) translate([0, 0, -41 * ForeArmCircumferenceScale]) thread_in(WristBoltDiameter, 40.5 * ForeArmCircumferenceScale);
}
...
module MakeThermoform3() {
    ...
    difference(){
        ...
        //Cut hole for first wrist bolt holder
        translate([ForeArmCircumferenceScale * 293.127, ForeArmCircumferenceScale * -6.568, ArmScale * -317.113]) rotate([0,90,0]) translate([0, 0, -90 * ForeArmCircumferenceScale])  cylinder(d=ElbowBoltDiameter, h=100 * ForeArmCircumferenceScale,center=false, $fn=20);
       
    }
    
    //Add inner threads for wrist bolt holder
    translate([ForeArmCircumferenceScale * 293.127, ForeArmCircumferenceScale * -6.568, ArmScale * -317.113]) rotate([0,90,0]) translate([0, 0, -77 * ForeArmCircumferenceScale]) thread_in(ElbowBoltDiameter, 77 * ArmScale);
}
